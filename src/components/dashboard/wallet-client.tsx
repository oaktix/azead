  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white font-heading">Wallet Ledger</h1>
          <p className="text-xs text-slate-400 mt-1">Fund your wallet or request manual withdrawals</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="p-1 rounded-xl bg-slate-900 border border-slate-800 flex">
            <button
              ...
              className={`flex-1 text-center py-2.5 rounded-lg text-xs font-bold transition-all ${
                activeTab === 'deposit' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
              Deposit Funds
            </button>
            <button
              ...
              className={`flex-1 text-center py-2.5 rounded-lg text-xs font-bold transition-all ${
                activeTab === 'withdraw' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-white'
              }`}
            >
              Request Withdrawal
            </button>
          ...
          <div className="p-6 sm:p-8 rounded-2xl bg-[#0b0f19] border border-slate-900 shadow-xl">
            {activeTab === 'deposit' ? (
              <form onSubmit={handleDeposit} className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white font-heading">Credit Wallet</h3>
                  <p className="text-xs text-slate-400 mt-1">Payments are processed instantly and credited automatically using Transactpay.</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white text-sm font-mono focus:outline-none focus:border-emerald-500"
                  ...
                  <div className="flex gap-2 mt-2">
                    {[100000, 500000, 1000000].map((amt) => (
                      <button
                        ...
                        className="text-[10px] bg-slate-950 text-slate-400 hover:text-white border border-slate-850 px-2.5 py-1 rounded"
                      >
              ...
              <form onSubmit={handleWithdrawal} className="space-y-5">
                <div>
                  <h3 className="text-lg font-bold text-white font-heading">Withdraw Funds</h3>
                  <p className="text-xs text-slate-400 mt-1">Withdrawals are processed manually within 24 hours. A transaction fee of 1.9% applies.</p>
                </div>
                ...
                {numWithdrawAmount > 0 && (
                  <div className="p-4 rounded-xl bg-slate-950 border border-slate-900 space-y-2 font-mono text-xs text-slate-400">
                    <div className="flex justify-between">
                      <span>Requested Gross Amount:</span>
                      <span className="text-white">{formatNaira(numWithdrawAmount)}</span>
                    </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 rounded-2xl bg-[#0b0f19] border border-slate-900 shadow-xl text-center space-y-2.5 relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-emerald-500/5 via-slate-950/0 to-slate-950/0" />
            ...
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Account Active Balance</div>
            <div className="text-3xl font-extrabold text-white font-mono">{formatNaira(balance)}</div>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-white font-heading px-1">Ledger Transaction Logs</h3>
            <div className="p-5 rounded-2xl bg-[#0b0f19] border border-slate-900 shadow-xl space-y-4 max-h-[360px] overflow-y-auto">
              {transactions.length === 0 ? (
                <p className="text-xs text-slate-500 text-center py-6">No transactions recorded.</p>
              ) : (
                    ...
                    return (
                      <div key={tx.id} className={`flex justify-between items-center ${idx > 0 ? 'pt-4' : ''}`}>
                        <div className="flex items-center space-x-2.5">
                          <div className={`p-1.5 rounded-lg flex-shrink-0 ${isCredit ? 'bg-emerald-950/50 text-emerald-400' : 'bg-red-950/50 text-red-400'}`}>
                            {isCredit ? <ArrowDownLeft className="w-3.5 h-3.5" /> : <ArrowUpRight className="w-3.5 h-3.5" />}
                          </div>
                          <div className="min-w-0">
                            <div className="text-xs font-semibold text-slate-200 capitalize truncate max-w-[125px]">{tx.description || tx.type.replace('_', ' ')}</div>
                            <div className="text-[8px] text-slate-500 mt-0.5 font-mono">{formattedDate}</div>
                          </div>
                        </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <span className="text-[8px] font-mono text-slate-500 uppercase bg-slate-950 px-1.5 py-0.5 rounded border border-slate-900/60 mt-0.5 inline-block">
                            {tx.type}
                          </span>
                        </div>
                      </div>
