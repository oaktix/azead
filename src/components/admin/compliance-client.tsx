...
export default function ComplianceClient({
  ...
  return (
    <div className="space-y-8">
      {/* Header section */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-foreground font-heading">Compliance Reviews</h1>
        <p className="text-xs text-muted mt-1">Approve investor KYC identity verifications and process pending bank withdrawals.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-base font-bold text-foreground font-heading px-1 flex items-center gap-2">
            <UserCheck className="w-5 h-5 text-emerald-400" />
            <span>KYC Verification Queue ({kycDocs.length})</span>
          </h3>

          <div className="p-6 rounded-2xl bg-card border border-border shadow-xl overflow-x-auto min-h-[200px] flex flex-col justify-between">
            {kycDocs.length === 0 ? (
                  ...
                  {kycDocs.map((doc) => {
                    ...
                    return (
                      <tr key={doc.id} className="align-middle">
                        ...
                        <td className="py-3.5 font-mono text-[10px] text-muted">
                          <div className="flex items-center gap-3">
                            <div 
                              ...
                              className="w-12 h-10 rounded border border-border bg-input overflow-hidden flex-shrink-0 cursor-pointer hover:border-emerald-500/50 transition-colors flex items-center justify-center"
                            ...
                            <div>
                              <span className="block text-xs font-semibold text-foreground">ID: {doc.id_number}</span>
                              <button 
                                onClick={() => {
                                  setActivePreviewDoc(doc);
                                  setIsPreviewOpen(true);
                          ...
                          {isRejecting ? (
                            <div className="flex items-center justify-end gap-2">
                              <input 
                                ...
                                className="bg-input border border-border rounded px-2 py-1 text-[10px] text-foreground focus:outline-none focus:border-primary"
                              />
                              <button 
                                title="Submit Rejection" 
                                onClick={() => handleProcessKYC(doc.id, doc.user_id, false)}
                                className="p-1 rounded bg-destructive/10 text-destructive text-[10px] font-semibold px-2 py-1 hover:bg-destructive hover:text-white transition-colors"
                              >
                                Submit
                              </button>
                              <button title="Cancel Rejection" onClick={() => setKycRejectId(null)}
                                className="p-1 rounded bg-input border border-border text-muted hover:text-foreground"
                              >
                            ...
                            <div className="flex justify-end gap-2">
                              <button title="Approve KYC" onClick={() => handleProcessKYC(doc.id, doc.user_id, true)}
                                  ...
                                  className="p-1.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-slate-950 transition-colors"
                              >
                              ...
                              <button title="Reject KYC" onClick={() => setKycRejectId(doc.id)}
                                  disabled={kycLoadingId !== null}
                                  className="p-1.5 rounded bg-destructive/10 border border-destructive/20 text-destructive hover:bg-destructive hover:text-white transition-colors"
                              >
                                <X className="w-3.5 h-3.5" />
                              </button>
                            </div>
      ...
      {isPreviewOpen && activePreviewDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-card border border-border rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6 my-8 animate-in fade-in zoom-in duration-200">
            <button 
              onClick={() => {
                setIsPreviewOpen(false);
                setActivePreviewDoc(null);
            ...
            <div>
              <h3 className="text-lg font-bold text-foreground font-heading">KYC Verification Review</h3>
            ...
            {/* Document display */}
            <div className="relative rounded-xl overflow-hidden border border-border bg-input flex items-center justify-center min-h-[300px] p-2">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img 
            ...
            {/* Applicant details */}
            <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-input border border-border font-mono text-xs">
              <div>
                <span className="block text-[10px] text-muted uppercase font-bold tracking-wider">Applicant</span>
              ...
              <div>
                <span className="block text-[10px] text-muted uppercase font-bold tracking-wider">ID / NIN Document Number</span>
            ...
            <div className="flex flex-col gap-4 border-t border-border pt-4">
              {kycRejectId === activePreviewDoc.id ? (
                <div className="space-y-3">
                  <span className="block text-xs font-semibold text-foreground">Explain rejection reason:</span>
                  <div className="flex gap-2">
                    <input 
                      ...
                      value={kycReason}
                      onChange={(e) => setKycReason(e.target.value)}
                      className="flex-1 bg-input border border-border rounded-xl px-4 py-2.5 text-xs text-foreground focus:outline-none focus:border-destructive"
                    />
                    <button 
                      onClick={async () => {
                    ...
                    <button 
                      ...
                      className="px-3 rounded-xl bg-input border border-border text-muted hover:text-foreground text-xs transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    disabled={kycLoadingId !== null}
                    className="flex-1 py-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive hover:bg-destructive hover:text-white font-bold text-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                    <X className="w-4 h-4" />
